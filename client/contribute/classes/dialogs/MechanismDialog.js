...
(function () { // self-invoking function
    ...
    SAS.MechanismDialog = function (/**SAS.MechanismDef*/ mechanism) {
        ...
        var _buildContent = function ($dlg) {
            ...
            var categoryOpts = [Enums.MECH_PROJECT, Enums.MECH_POLICY];
            $('<label for="categorys_sel">Category:</label>').addClass("dialogLabel").appendTo($inputsDiv);
            _$category = $('<select id="categorys_sel"></select>').appendTo($inputsDiv);
            SAS.controlUtilsInstance.populateSelectList(_$category, null, categoryOpts, _mechanism.category);
